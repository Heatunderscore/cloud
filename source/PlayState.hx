...
class PlayState extends MusicBeatState
{
	...
	public static var cloudGameOver:Bool = false;
	...
	var gameMode:String = isStoryMode ? 'Story Mode' : 'Freeplay';

	override public function update(elapsed:Float)
	{
		Application.current.window.title = Main.initTitle + " | " + SONG.song + ' - ' 
		+ CoolUtil.difficulties[storyDifficulty] + ' | Score: ' + songScore + ' | Misses: ' + songMisses + ' | Rating: '
		+ ratingName + ' | ' + gameMode;
	...
	function doDeathCheck(?skipHealthCheck:Bool = false) {
		if (((skipHealthCheck && instakillOnMiss) || health <= 0) && !practiceMode && !isDead)
		{
			...
			if(ret != FunkinLua.Function_Stop) {
				...
				cloudGameOver = SONG.player2 == 'tiredcloud'; 
			...
			case 'Countdown':
				...
				startTimer = new FlxTimer().start(Conductor.crochet / 1000, function(tmr:FlxTimer)
					{	
						...
						{
							case 0:
								if (SONG.song.toLowerCase() != 'hurricane')
									FlxG.sound.play(Paths.sound('intro3' + introSoundsSuffix), 0.6);
							case 1:
								countdownReady = new FlxSprite().loadGraphic(Paths.image(introAlts[0]));
								countdownReady.scrollFactor.set();
		...
		if(ret != FunkinLua.Function_Stop && !transitioning) {
			...
			if (isStoryMode)
			{
				...
				else
				{
					...
					var white:FlxSprite = new FlxSprite(0, 0).makeGraphic(FlxG.width * 5, FlxG.height * 5, 0xffb9c7);
					white.alpha = 0;
					add(white);

					var loadingSprite:FlxSprite = new FlxSprite(0, 0).loadGraphic(Paths.image('funkay'));
					loadingSprite.scale.set(0.7, 0.7);
	...
	private function popUpScore(note:Note = null):Void
	{
		...
		if(daRating == 'sick' && !note.noteSplashDisabled && ClientPrefs.noteSplashes)
		{
	...
	function opponentNoteHit(note:Note):Void
	{
		...
		if(!note.isSustainNote && ClientPrefs.cpuSplashes)
			spawnNoteSplashOnNote(note);
