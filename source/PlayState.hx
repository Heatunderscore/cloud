...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		switch (curStage)
		{
			...
			case 'sonicfunstage': 
			    var bg:FlxSprite = new FlxSprite().loadGraphic(Paths.image('sonicfunstage'));
			    add(bg);
				bg.setPosition(-600, -200);

			case 'house': 
			    var bg:FlxSprite = new FlxSprite().loadGraphic(Paths.image('housePlace'));
			    add(bg);
				bg.setPosition(-600, -200);
				var bgCharacter:BGSprite = new BGSprite('backgroundguys', 0, 100);
				bgCharacter.animation.addByPrefix('dance', 'Upper Crowd Bob', 24);
				add(bgCharacter);
				bgCharacter.dance(true);


			case 'wethouse': 
			    var bg:FlxSprite = new FlxSprite().loadGraphic(Paths.image('housePlaceRain'));
			    add(bg);
				bg.setPosition(-600, -200);
		...
		if(ret != FunkinLua.Function_Stop && !transitioning) {
			...
			if (isStoryMode)
			{
				...
				else
				{
					...
					//FlxTween.tween(loadingSprite, {alpha: 100}, 1);
